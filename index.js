...
function checkSelection(selectedWidgets) {
  if (selectedWidgets.length === 1 
    ...
    && selectedWidgets[0].metadata[APP_ID].isDownArrow
    && selectedWidgets[0].metadata[APP_ID].igUserID){
      if (getIgUserContent){
          getIgUserContent(selectedWidgets[0].metadata[APP_ID].igUserID, showIgUserContent);
      }
  }
}
...
function generateIgPosts(posts, baseShape, downArrow){
  let widgets = [];
  for (var i = 0; i < posts.length / 3  ; i++) {
    for (var j = 0; j < 3 && j + i * 3 < posts.length ; j++) {
      let post = posts[ i * 3 + j ].node;
      if (post.display_url){
        widgets.push(getIgPost(post.display_url, (j-1) * (postSize + 40), (i + 1) * (postSize + 20) - 10));
      }
    }
  }
  ...
  miro.board.widgets.create(widgets).then(wdgts => {
    ...
    for (var k = 0; k < wdgts.length; k++){
      ...
      let heightDelta = Math.ceil(posts.length / 3) * (postSize +40);
...
function showIgPosts(posts, userID){
  miro.board.widgets.get({type: 'shape'}).then(shapes => {
    let baseShapeExisted = shapes.filter(shape => isBaseShapeWidget(shape) && isUserWidget(shape, userID));
    if (baseShapeExisted && baseShapeExisted.length){
      miro.board.widgets.get({type: 'image', url:doubleArrowURL }).then(images => {
        let downArraw = images.filter(image => isDownArrowWidget(image) && isUserWidget(image, userID));
        if (downArraw && downArraw.length){
          generateIgPosts(posts, baseShapeExisted[0], downArraw[0]);
        }
      });
    } else {
...
function showIgUserContent(userContent, userID){
    if (userContent && userContent.status==='ok' && miro){
        showIgPosts(userContent.data.user.edge_owner_to_timeline_media.edges, userID);
        //save somewhere userContent.data.user.edge_owner_to_timeline_media.page_info
    }
}